...
8. **Check resource tree**:
   ```bash
   ktree pod <pod-name>
   ```

**AI Agent Notes**: Use `stern` for multi-pod deployments. `kubectl-tree` helps understand resource relationships.
...
2. **Clean manifest** (optional, for cleaner diffs):
   ```bash
   kneat < deployment.yaml > deployment-clean.yaml
   ```

3. **Validate with kubectl**:
   ```bash
...
2. **Clean manifest**:
   ```bash
   k get deployment <name> -o yaml | kneat > deployment-clean.yaml
   ```
...
**AI Agent Notes**: Use `kubectl-neat` to get clean manifests for version control.
...
1. **View resource tree**:
   ```bash
   ktree deployment <deployment-name>
   ```

2. **Get clean manifest**:
   ```bash
   k get deployment <name> -o yaml | kneat > deployment.yaml
   bat deployment.yaml
   ```
...
**AI Agent Notes**: `kubectl-tree` provides visual representation of resource hierarchies. Use `kubectl-neat` for clean manifests.
